...
public class EsTemplate extends AbstractESOperation {
	...
	public String getIndex(ESQuery query) {
		...
		if(StringUtil.isEmpty(index)){
			...
			if ("day".equals(dateType)) {
				index = prefix + DateUtil.getTodayStr("yyyyMMdd");
			} else {
				index = prefix + "_top_" + dateType;
			}
		}
	...
	public JTableData queryTable(ESQuery query) {
		...
		// Summary Data
		Map<String, Object> sumData = getSumMap(items[0].getResponse(), query);
		tableData.addSum(sumData);
		
		// Count
		long records = new Long(sumData.remove("records").toString());
		records = Math.min(max, records);
